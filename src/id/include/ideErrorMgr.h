...
/**
 * Exception section에서 IDE_FT_BEGIN()에 대한 종료로 IDE_FT_END_FAILURE()
 * 매크로 사용. FATAL 극복 구간에서 exception section으로 jump시 설계 의도대로
 * FATAL 극복시키지 않는데 이를 해제한다. IDE_FT_END()와 마찬가지로 이후
 * FATAL 극복여부는 caller에 의존한다.
 */
#ifdef DEBUG
