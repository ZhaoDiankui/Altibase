 ...
 *
 * SockType에 따라 Select, Poll에 맞는 함수를 설정한다.
 */
IDE_RC cmnDispatcherInitialize()
{
    cmnDispatcherAllocInfo sCmnDispatcherAllocInfoPoll =
    {
#if defined(CM_DISABLE_TCP) && defined(CM_DISABLE_UNIX)
        NULL,
        NULL
#else
        cmnDispatcherMapSOCKPoll,
        cmnDispatcherSizeSOCKPoll
#endif
    };

    IDE_RC (*sCmnDispatcherWaitLinkPoll)(cmnLink *aLink,
                                         cmnDirection aDirection,
                                         PDL_Time_Value *aTimeout) =
    {
#if defined(CM_DISABLE_TCP) && defined(CM_DISABLE_UNIX)
    ...
    sSockPollType =
        (cmnDispatcherSockPollType) cmuProperty::getCmDispatcherSockPollType();
    ...
    switch (sSockPollType)
    {
        ...
        case CMN_DISPATCHER_SOCK_POLL:
            ...
            gCmnDispatcherAllocInfo[sImpl] = sCmnDispatcherAllocInfoPoll;
            gCmnDispatcherWaitLink[sImpl]  = sCmnDispatcherWaitLinkPoll;
            break;
