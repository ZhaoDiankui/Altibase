...
static void cmnLinkPeerDestroySslCtx(cmnLinkPeer *aLink)
{
    ...
    cmnOpenssl::mFuncs.SSL_COMP_free_compression_methods();
#endif

    /* Exit the FIPS mode of operation */
    (void)cmnOpenssl::mFuncs.FIPS_mode_set(0);

    /* Release shared global data structures */
    cmnOpenssl::mFuncs.CRYPTO_set_locking_callback(NULL);
    cmnOpenssl::mFuncs.CRYPTO_set_id_callback(NULL);
    cmnOpenssl::mFuncs.CRYPTO_cleanup_all_ex_data();

    /* Free error strings */
    cmnOpenssl::mFuncs.ERR_free_strings();
    ...
    cmnOpenssl::mFuncs.EVP_cleanup();
    
    /* Unload ENGINE objects */
    cmnOpenssl::mFuncs.ENGINE_cleanup();

    /* Finish and unload configuration modules (1: All modules) */
    cmnOpenssl::mFuncs.CONF_modules_unload(1);
}
