...
extern cmnOpensslFuncs *gOpenssl;
...
static ACI_RC cmnLinkPeerDestroySslCtx(cmnLinkPeer *aLink)
{
    ...
    {
        gOpenssl->SSL_CTX_free( sLink->mDesc.mSslCtx );
                                           ...
                                           cmnLinkConnectArg *aConnectArg )
{
    ...
    sMethod = (SSL_METHOD *)gOpenssl->TLSv1_client_method();

    /* create new context from the method */
    sSslCtx = gOpenssl->SSL_CTX_new(sMethod);
    ACI_TEST_RAISE(sSslCtx == NULL, SslOperationFail );

    /* set the certificate */
    if (sCert != NULL)
    {
        sSslRet = gOpenssl->SSL_CTX_use_certificate_file(sSslCtx, sCert, SSL_FILETYPE_PEM);
    ...
    if (sKey != NULL)
    {
        sSslRet = gOpenssl->SSL_CTX_use_PrivateKey_file(sSslCtx, sKey, SSL_FILETYPE_PEM);
        ACI_TEST_RAISE(sSslRet == 0, FailedToSetCertificate);
    }
    else
    {
    ...
    {
        sSslRet = gOpenssl->SSL_CTX_check_private_key(sSslCtx);
    ...
    if (sCipher != NULL)
    {
        sSslRet = gOpenssl->SSL_CTX_set_cipher_list(sSslCtx, sCipher);
    ...
    {
        sRet = gOpenssl->SSL_CTX_load_verify_locations(sSslCtx, sCa, sCaPath);
    ...
    else
    {
        ...
        sRet = gOpenssl->SSL_CTX_set_default_verify_paths(sSslCtx);
        ACI_TEST_RAISE(sRet == 0, FailedToLoadVerifyLocations);
    }

    if (sVerify == ACP_TRUE)
    {
        gOpenssl->SSL_CTX_set_verify(sSslCtx, SSL_VERIFY_PEER, NULL);
    }
    else
    {
        gOpenssl->SSL_CTX_set_verify(sSslCtx, SSL_VERIFY_NONE, NULL);
    }

    gOpenssl->SSL_CTX_set_session_cache_mode(sSslCtx, SSL_SESS_CACHE_OFF);
    ...
    (void)gOpenssl->SSL_CTX_ctrl(sSslCtx, SSL_CTRL_MODE, SSL_MODE_AUTO_RETRY, NULL);
...
ACI_RC cmnLinkPeerCloseSSL(cmnLink *aLink)
{
    ...
    {
        gOpenssl->SSL_set_quiet_shutdown(sLink->mDesc.mSslHandle, 1);

        /* remove the SSL structure and free up the allocated memory */
        gOpenssl->SSL_free(sLink->mDesc.mSslHandle);
        sLink->mDesc.mSslHandle = NULL;
    }
    else
    {
                             ...
                             acp_sint32_t       aOption)
{
    ...
    sLink->mDesc.mSslHandle = gOpenssl->SSL_new(sLink->mDesc.mSslCtx);
    ACI_TEST_RAISE(sLink->mDesc.mSslHandle == NULL, SslOperationFail);

    sSslRet = gOpenssl->SSL_set_fd(sLink->mDesc.mSslHandle, sLink->mDesc.mSock.mHandle);
    ACI_TEST_RAISE(sSslRet != 1, SslOperationFail); /* 0: failed, 1: succeeded */

    sSslRet = gOpenssl->SSL_connect(sLink->mDesc.mSslHandle);
                              ...
                              cmnShutdownMode aMode)
{
    ...
    if(sLink->mDesc.mSslHandle != NULL)
    {
        ...
        sRet = gOpenssl->SSL_shutdown(sLink->mDesc.mSslHandle);
        if (sRet == 0) 
        {
            /* BUG-39922 */
            gOpenssl->SSL_set_quiet_shutdown(sLink->mDesc.mSslHandle, 1);
                             ...
                             acp_time_t      aTimeout)
{
    ...
    while (aBlock->mDataSize < aSize)
    {
         ...
         * Socket으로부터 읽음
         */
        sSize = gOpenssl->SSL_read(sLink->mDesc.mSslHandle,
                                   aBlock->mData + aBlock->mDataSize,
                                   aBlock->mBlockSize - aBlock->mDataSize);

        if (sSize <= 0)
        {
            sSslError = gOpenssl->SSL_get_error(sLink->mDesc.mSslHandle, sSize);
                             ...
                             acp_time_t      aTimeout)
{
    ...
    while (aBlock->mCursor < aBlock->mDataSize)
    {
        ...
        sSize = gOpenssl->SSL_write(sLink->mDesc.mSslHandle,
