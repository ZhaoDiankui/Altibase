...
#define GET_TRIGGER_QUERY "SELECT a.user_name, a.trigger_oid, b.seqno, b.substring "       \
                           "from system_.sys_triggers_ a, system_.sys_trigger_strings_ b " \
                           "where b.trigger_oid = a.trigger_oid "                          \
                           "and a.USER_NAME=? "                                            \
                           "order by 1, 2, 3 "

#define GET_TRIGGER_SYS_QUERY "SELECT a.user_name, a.trigger_oid, b.seqno, b.substring "   \
                           "from system_.sys_triggers_ a, system_.sys_trigger_strings_ b " \
                           "where b.trigger_oid = a.trigger_oid "                          \
                           "order by 1, 2, 3 "
...
#define IDE_FN "getTrigQuery()"
    ...
    IDE_TEST_RAISE(    
    SQLBindCol( s_trig_stmt, 2, SQL_C_SBIGINT,
                &s_trig_id, 0, NULL )
                != SQL_SUCCESS, trig_error );
